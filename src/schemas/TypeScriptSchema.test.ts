...
describe('TypeScriptConfig', () => {
  ...
  it('should fail to parse file with no default export', async () => {
    const filePath = `${__dirname}/../resources/schema/invalid-schema-1.ts`;

    const parser = new TypeScriptSchema(filePath);
    ...
    expect(parsed.errorMessage).toEqual(
      `File ${filePath} does not export schema as default`,
    );
  });

  it('should fail to parse file with improper schema type', async () => {
    const filePath = `${__dirname}/../resources/schema/invalid-schema-2.ts`;
    const parser = new TypeScriptSchema(filePath);
